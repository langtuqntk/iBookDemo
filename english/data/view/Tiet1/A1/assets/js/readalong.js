var ReadAlong = {
    ...
    generateWordList: function () {
        ...
        this.words = Array.prototype.map.call(word_els, function (word_el, index) {
            ...
            word.index = index;
            word.end = word.dur + word.begin;
            word_el.dataset.index = word.index;
            return word;
        });
    },

    generateWordHHList: function () {
        var word_els = this.text_hoathinh.querySelectorAll('[data-begin]');
        this.wordsHH = Array.prototype.map.call(word_els, function (word_el, index) {
            var word = {
                'begin': parseFloat(word_el.dataset.begin),
                'dur': parseFloat(word_el.dataset.dur),
                'element': word_el
    ...
    getCurrentWordHH: function () {
        var i;
        var len;
        var is_current_word;
        var word = null;
        for (i = 0, len = this.wordsHH.length; i < len; i += 1) {
            is_current_word = (
                (
                    this.audio_element.currentTime >= this.wordsHH[i].begin
                    &&
                    this.audio_element.currentTime < this.wordsHH[i].end
                )
                ||
                (this.audio_element.currentTime < this.wordsHH[i].begin)
            );
    ...
    selectCurrentWord: function() {
        ...
        var current_word = this.getCurrentWord(),
            current_wordHH = this.getCurrentWordHH();
        if(!current_word){
            return;
        }
        ...
        if (!current_word.element.classList.contains('speaking')) {
            if(that.audio_element.currentTime >= this.words[0].begin - 3){
                ...
                current_wordHH.element.classList.add('speaking');
            }
    ...
    removeWordSelection: function() {
        ...
        var spoken_word_els = this.text_element.querySelectorAll('span[data-begin].speaking'),
            spoken_word_elsHH = this.text_hoathinh.querySelectorAll('span[data-begin].speaking');
        Array.prototype.forEach.call(spoken_word_els, function (spoken_word_el) {
            spoken_word_el.classList.remove('speaking');
        });
        Array.prototype.forEach.call(spoken_word_elsHH, function (spoken_word_elHH) {
            spoken_word_elHH.classList.remove('speaking');
        });
    },
    ...
    addEventListeners: function () {
        ...
        that.audio_element.addEventListener('pause', function (e) {
            ...
            that.text_hoathinh.classList.remove('speaking');
        }, false);
